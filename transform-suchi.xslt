...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
    ...
    xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:step="http://www.stibosystems.com/step"
    ...
    <xsl:template match="/fyayc:people">
        <step:STEP-ProductInformation xmlns="http://www.stibosystems.com/step"
            xsi:schemaLocation="http://www.stibosystems.com/step PIM.xsd"
            STEPWorkflowImportEvent="Y" UseContextLocale="false" WorkspaceID="Main"
            SingleUpdateMode="N">
            ...
            <step:Products>
                <step:Product ID="TEAM_Home" ParentID="Product hierarchy root"
                    UserTypeID="TEAM_Home">
                    <xsl:apply-templates/>
                </step:Product>
            </step:Products>
        ...
        <step:Product UserTypeID="TEAM_Leaf">
            ...
            <step:Values>
                <xsl:for-each select="@*[local-name() != 'when']">
                    ...
                    <step:Value>
                        <xsl:attribute name="AttributeID" select="concat('FYAYC_', $attribute)"/>
                        <xsl:value-of select="."/>
                    </step:Value>
                </xsl:for-each>
                <step:Value AttributeID="FYAYC_when">
                    <xsl:call-template name="date_fixer">
